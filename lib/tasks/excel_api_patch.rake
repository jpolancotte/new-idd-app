namespace :db do
    namespace :seed do
      desc "convert string to lower cas underscore"
      task excel_api_patch: :environment do
        
        require 'open-uri'
        require 'date'
        require 'uri'
        require 'net/http'
        require 'openssl'
        require 'json'
        require 'csv'
        require 'pp'
        
        require "csv"
        require 'nokogiri'
        require 'httparty'
        require 'json'


        #str = "https://graph.microsoft.com/v1.0/me/drive/items/01OPZEJ6PLMYAKD65ODNHL6TX6YGUBW7N2/workbook/worksheets('Sales Pipeline')/pivotTables/D56BB33A-4304-46CF-A6CC-CBF7EE956764"  #get pivot table ID


        #str = "https://graph.microsoft.com/v1.0/me/drive/items/01OPZEJ6PLMYAKD65ODNHL6TX6YGUBW7N2/workbook/worksheets('Sales Pipeline')/pivotTables"  #get pivot table ID


        #str="https://graph.microsoft.com/v1.0/me/drive/items/01OPZEJ6PLMYAKD65ODNHL6TX6YGUBW7N2/workbook/worksheets('Master Data')/range(address='A377:T377')"


        
        #str = "https://graph.microsoft.com/v1.0/me/drive/items/01OPZEJ6PLMYAKD65ODNHL6TX6YGUBW7N2/workbook/worksheets('Master Data')"

        #str = "https://graph.microsoft.com/v1.0/me/drive/items/01OPZEJ6PLMYAKD65ODNHL6TX6YGUBW7N2/workbook/tables/table1" #tables
        #str = "https://graph.microsoft.com/v1.0/me/drive/items/01OPZEJ6PLMYAKD65ODNHL6TX6YGUBW7N2/workbook/tables/table1" #List tables

        #str = "https://graph.microsoft.com/v1.0/me/drive/root:/JP Pipleline TEST.xlsx:/workbook/worksheets/Master Data/tables/table1/rows/"


        str = "https://graph.microsoft.com/v1.0/me/drive/root:/JP Pipleline TEST.xlsx:/workbook/worksheets/Master Data/range(address='O379:P379')" # add table row
        encoded = URI::DEFAULT_PARSER.escape(str)
        # puts encoded       

        a="eyJ0eXAiOiJKV1QiLCJub25jZSI6InF4T2FvWHVvRnZ3U2g0QlNlWjVoSV9vZXlzSmktd3gxNjA2UU1iRVVEc28iLCJhbGciOiJSUzI1NiIsIng1dCI6Ii1LSTNROW5OUjdiUm9meG1lWm9YcWJIWkdldyIsImtpZCI6Ii1LSTNROW5OUjdiUm9meG1lWm9YcWJIWkdldyJ9..rdK7rE2lXGblARcdEnWGvhm2qH8kLt153AwczTxR7X-Ey6b1C2ljOGrQx6DLqogHoEw2bJi1fZAFMNoKuu68DnjLjv6ezyN2oRMqsPol5OpJJJ9iWhWdB2t07vXtRDlxnOkOo8D5pVsdmTt3DRjTozR66C6pS65U5yjprrW0G8YiUQZLf9F_un6tanueRAxNgnru1e3kiH4jUNFCjAOIBDWzAmFIBHpBP5iwfQhAopQJzxHifMbouqrVUE-umQUoP-4QzCfrq6Ol__2-kHhju-HiEBLLq9HLcGRpkL6In3bFWq5WrPMyb1FQV2Vjtp2TxLs4YGuvm-g94ZsmNamQMQ"

               
        response = HTTParty.post("#{encoded}",
        :body => 
          {  "values"=>
                      [
                        ["Kathy Brown / Nanette Wrobel", "Apostolic EIHA", "", 
                        "Onboarding - Complete", "NJ", "NY", 70, 1, 44166, 
                        "PAL", "", "", "", "","=IFERROR(EDATE(I377,-1),"")", "=EDATE(I377,1)",
                         "=VLOOKUP(I377,Date_Lookups[#All],14,FALSE)",
                          "#{nil}",
                          "#{nil}",
                          ""
                        ]
                      ],
                      
            "formulas"=>[["#{nil}", "#{nil}", "#{nil}", 
            "#{nil}", "#{nil}", "#{nil}", "#{nil}", "#{nil}", "#{nil}", 
            "#{nil}", "#{nil}", "#{nil}", "#{nil}", "#{nil}", "=IFERROR(EDATE(I377,-1),"")","=EDATE(I377,1)", "=VLOOKUP(I377,Date_Lookups[#All],14,FALSE)", "#{nil}", "#{nil}", "#{nil}"]]
                    
          }.to_json,

        :headers => {'Content-Type' => 'application/json', "Authorization" => "Bearer #{a}" } )

        
        
       pp response
      
       
        
        
      end
    end
  end