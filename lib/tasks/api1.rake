namespace :db do
    namespace :seed do
      desc "convert string to lower cas underscore"
      task api1: :environment do
        
        require 'open-uri'
        require 'date'
        require 'uri'
        require 'net/http'
        require 'openssl'
        require 'json'
        require 'csv'
        require 'pp'
        
        require "csv"
        require 'nokogiri'
        require 'httparty'
        require 'json'


        #str = "https://graph.microsoft.com/v1.0/me/drive/items/01OPZEJ6PLMYAKD65ODNHL6TX6YGUBW7N2/workbook/worksheets('Sales Pipeline')/pivotTables/D56BB33A-4304-46CF-A6CC-CBF7EE956764"  #get pivot table ID


        #str = "https://graph.microsoft.com/v1.0/me/drive/items/01OPZEJ6PLMYAKD65ODNHL6TX6YGUBW7N2/workbook/worksheets('Sales Pipeline')/pivotTables"  #get pivot table ID


        #str="https://graph.microsoft.com/v1.0/me/drive/items/01OPZEJ6PLMYAKD65ODNHL6TX6YGUBW7N2/workbook/worksheets('Master Data')/range(address='A377:T377')"


        
        #str = "https://graph.microsoft.com/v1.0/me/drive/items/01OPZEJ6PLMYAKD65ODNHL6TX6YGUBW7N2/workbook/worksheets('Master Data')"

        #str = "https://graph.microsoft.com/v1.0/me/drive/items/01OPZEJ6PLMYAKD65ODNHL6TX6YGUBW7N2/workbook/tables/table1" #tables
        #str = "https://graph.microsoft.com/v1.0/me/drive/items/01OPZEJ6PLMYAKD65ODNHL6TX6YGUBW7N2/workbook/tables/table1" #List tables

        str = "https://graph.microsoft.com/v1.0/me/drive/root:/JP Pipleline TEST.xlsx:/workbook/worksheets/Master Data/tables/table1/rows" #add

       

        #str = "https://graph.microsoft.com/v1.0/me/drive/root:/JP Pipleline TEST.xlsx:/workbook/tables/table1/rows/380" #delete

        #str = "https://graph.microsoft.com/v1.0/me/drive/root:/JP Pipleline TEST.xlsx:/workbook/worksheets/Master Data/range(address='A378:T378')/delete" #delete works

        # str = "https://graph.microsoft.com/v1.0/me/drive/root:/JP Pipleline TEST.xlsx:/workbook/worksheets/Sales Pipeline/pivotTables/refreshAll"

       
       

       # str = "https://graph.microsoft.com/v1.0/me/drive/root:/JP Pipleline TEST.xlsx:/workbook/worksheets/Master Data/tables/table1/rows/#{patch}" #patch 

        #str = "https://graph.microsoft.com/v1.0/me/drive/root:/JP Pipleline TEST.xlsx:/workbook/worksheets/Master Data/range(address='A378:T378')" # patch
        encoded = URI::DEFAULT_PARSER.escape(str)
        # puts encoded  
        
        
        
       

        a="eyJ0eXAiOiJKV1QiLCJub25jZSI6InQwQVBtYlIwMTNoelY1S0dOOGxaV2h0SnBUaWJvaGN0Y3V2Y1FaUGlIRlEiLCJhbGciOiJSUzI1NiIsIng1dCI6Ii1LSTNROW5OUjdiUm9meG1lWm9YcWJIWkdldyIsImtpZCI6Ii1LSTNROW5OUjdiUm9meG1lWm9YcWJIWkdldyJ9..gE0DNY_8grf8oHNA66kzpH9WZ11JsdrP0Y9uUtnZGaileZ9VYfYWp4B_Ui4gJC3GjfHH20HT01HRn2mPqz3HEN-qLch8BgS6vy45V_wujNhvOlvw-vhHOLnplHhkiZjZLA0u6rQZQRRv1hLeo2HQ0dFOaU6XqK1GbI7t1ABn2zmSHvmhzYEgPNGHPYG_-GpZiUJxFDB-wDxju5Lzl0wNw4XMgii_bExM1OQ37pxnDNL4a6_nhtL23K0taaumuwppoeZxVz1w55YjCViyeU-RBA3qLwVaO-NpFcgIXEIy-kklUGS9Umt6Hw-FhTVjtgeAhXQWkCLbCY6vyp_T_UZecw"
       
               
        # response = HTTParty.get("#{encoded}",
        #   :headers => {'Content-Type' => 'application/json', "Authorization" => "Bearer #{a}" } )        
        
          # total = response["value"].size + 2

          total = patch + 2
          puts total

          # pp response


          

          os='"Onboarding - Sales"'
          op='"Onboarding - In Progress"'
          oc = '"Onboarding - Complete"'
          y='"Y"'
          n='"N"'

       #"=IF(I#{total}=#{os},#{y},IF(I#{total}=#{op},#{y},IF(I#{total}=#{oc},#{y},#{n})))", 

       #'=IF(D377="Onboarding - Sales","Y",IF(D377="Onboarding - In Progress","Y",IF(D377="Onboarding - Complete","Y","N")))',

            # status="=IF(D#{total}=#{os},#{y},IF(D#{total}=#{op},#{y},IF(D#{total}=#{oc},#{y},#{n})))"   

            
       response2 = HTTParty.patch("#{encoded}",
        :body => 
          {  "values"=>
                      [
                        ["Kathy Brown / Nanette Wrobel", "10A Apostolico Now EIHA", "", 
                        "Sales - Initial Communication", "NJ", "NJ", 100, 0.75, 44166,
                        "PAL", "", "", "", "",
                        "=IFERROR(EDATE(I#{total},-1),"")", 
                        "=EDATE(I#{total},1)",
                        "=VLOOKUP(I#{total},Date_Lookups[#All],14,FALSE)",
                        "=VLOOKUP(I#{total},Date_Lookups[#All],6,FALSE)", 
                        "=IF(D#{total}=#{os},#{y},IF(D#{total}=#{op},#{y},IF(D#{total}=#{oc},#{y},#{n})))" , 
                        "=YEAR([@[Go Live'' Date]])"
                        ]
                      ]
                      
                     
                    
          }.to_json,

        :headers => {'Content-Type' => 'application/json', "Authorization" => "Bearer #{a}" } )       

      pp response2


      # str1 = "https://graph.microsoft.com/v1.0/me/drive/root:/JP Pipleline TEST.xlsx:/workbook/worksheets/Sales Pipeline/pivotTables/refreshAll"
      # encoded = URI::DEFAULT_PARSER.escape(str1)
               
      # response3 = HTTParty.post("#{encoded}",      
      # :headers => {'Content-Type' => 'application/json', "Authorization" => "Bearer #{a}" } )
            
      # pp response3    

        
        
      end
    end
  end